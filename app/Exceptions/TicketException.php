...
class TicketException extends ToolboxException
{
    /**
     * Hands back what IS allowed, which turns a dead end into something the UI
    ...
    {
        return new self("This ticket is already {$status->label()}.", 'TICKET_ALREADY_IN_STATUS', 409, ['status' => $status->value]);
    }

    public static function reopenReasonRequired(): self
    {
        return new self('Reopening a ticket needs a reason.', 'TICKET_REOPEN_REASON_REQUIRED', 422);
    }
    ...
    {
        return new self("\"{$key}\" is no longer accepting new tickets.", 'TICKET_SECTION_INACTIVE', 422, ['section_key' => $key]);
    }
    ...
    {
        return new self('That parent would put the level inside itself.', 'TICKET_LEVEL_CYCLE', 422, ['path' => $path]);
    }

    public static function assignmentTargetRequired(): self
    {
        return new self('An assignment needs either a section or a level.', 'TICKET_ASSIGNMENT_TARGET_REQUIRED', 422);
    }

    public static function assignmentTargetAmbiguous(): self
    {
        return new self('An assignment takes a section or a level, not both.', 'TICKET_ASSIGNMENT_TARGET_AMBIGUOUS', 422);
    }

    public static function assignmentDuplicate(): self
    {
        return new self('That user is already assigned here.', 'TICKET_ASSIGNMENT_DUPLICATE', 409);
    }

    public static function participantRedundant(string $why): self
    {
        return new self($why, 'TICKET_PARTICIPANT_REDUNDANT', 422);
    }
}
