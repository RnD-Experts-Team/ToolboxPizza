...
class UserUpdatedHandler implements EventHandlerInterface
{
    public function handle(array $event): void
    {
        ...
        DB::transaction(function () use ($id, $nameTo, $emailTo) {
            ...
            // Query Builder, not Eloquent, on purpose: this bypasses casts,
            // mutators, observers and package hooks, so a malformed value in a
            // replayed event cannot trip downstream model logic ("Array to
            // string conversion" was the original symptom). Replication should
            // write exactly what the bus said and nothing else.
            DB::table('users')
                ->where('id', $id)
                ->update($update);
        });
